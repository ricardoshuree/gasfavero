// [mcp-local harness] feature: inadimplentes-motorista-fix | plano: 458a0339 | 2026-09-07 13:23:40
// Remove useRef, FiadoEmAberto e dias nao utilizados
// Tela de Inadimplentes — app motorista
// Toggle meus/todos, busca por nome/CPF, ordenado por dias de atraso (maior primeiro)
...
// ---------------------------------------------------------------------------
// Sheet de confirmação de recebimento (inline)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Sheet de detalhe do inadimplente
// ---------------------------------------------------------------------------
function DetalheSheet({
  venda,
  token,
  onFechar,
  onRecebimentoConfirmado,
}: {
  venda: InadimplentVenda
  token: string
  onFechar: () => void
  ...
  return (
    <div style={ss.overlay} onClick={onFechar}>
      <div style={ss.sheet} onClick={e => e.stopPropagation()}>
        ...
        <button style={s.btnPagar} onClick={() => setConfirmar(true)}>
          💰 Receber pagamento
        </button>
...
export default function InadimplentesTola({ token, usuario }: Props) {
  ...
  const filtrados = todos
    .filter(v => somenteMeus ? v.motorista_id === usuario.id : true)
    .filter(v => {
    ...
    <div style={s.pagina}>
        ...
        <div style={s.toggleRow}>
          <div style={s.toggleWrap} onClick={() => setSomenteMeus(p => !p)}>
            ...
            <span style={s.toggleLabel}>
              {somenteMeus ? "Somente meus" : "Todos os vendedores"}
            </span>
          </div>
      ...
      {!carregando && filtrados.length === 0 && (
        <div style={s.vazio}>
          ...
          <p style={s.vazioSub}>
            {somenteMeus ? "Seus clientes estão em dia." : "Nenhum fiado em atraso no momento."}
          </p>
        </div>
      )}
              ...
              <div style={s.cardHeader}>
                <div style={{ flex: 1 }}>
                  <div style={s.cardNome}>
                    {v.cliente_nome}
                    {ehMeu && !somenteMeus && <span style={s.badgeMeu}>meu</span>}
                  </div>
                  <div style={s.cardSub}>
