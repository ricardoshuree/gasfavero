// [mcp-local harness] feature: fix-toggle-todos-meus | plano: 09bda2e0 | 2026-09-07 19:41:34
// Toggle invertido: ligado (verde) = todos os motoristas, desligado = somente meus. Estado inicial false = meus.
// Tela de Devolução de Cascos — app motorista
// Lista cascos emprestados (em aberto, sem confirmação do gerente)
// Toggle todos/meus · sheet de detalhe · registrar devolução física (step 1)
          ...
          <>
            ...
            {jaRecebido ? (
              <div style={ss.avisoAzul}>
                ✓ Devolução registrada. Aguardando confirmação do gerente no sistema web.
              </div>
            ) : (
    ...
    <div style={s.pagina}>
        ...
        <div style={s.toggleRow}>
          ...
          <div style={s.toggleWrap} onClick={() => setVerTodos(p => !p)}>
            ...
            <span style={s.toggleLabel}>
              {verTodos ? "Todos os motoristas" : "Somente meus"}
            </span>
          </div>
      ...
      {!carregando && todos.length === 0 && (
        <div style={s.vazio}>
          ...
          <p style={s.vazioSub}>
            {verTodos ? "Nenhum botijão emprestado no momento." : "Todos os seus botijões foram devolvidos."}
          </p>
        </div>
      )}
...
function CardCasco({ casco, onTap }: { casco: Casco; onTap: () => void }) {
  ...
  return (
    <div
      style={{ ...s.card, borderColor: urgente ? "#fca5a5" : "#e5e7eb", borderWidth: urgente ? "1.5px" : "1px" }}
      onClick={onTap}
    >
