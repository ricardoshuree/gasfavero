...
import {
  buscarClientesFiado, buscarMeusFiados, diasEmAberto,
  isAtrasado, marcarFiadoRecebido,
  type ClienteBusca, type FiadoEmAberto,
} from "../lib/fiado"
...
function formatEndereco(end: FiadoEmAberto["endereco"]): string | null {
  if (!end) return null
...
function ConfirmarSheet({
  ...
  async function confirmar() {
    ...
    } catch (e: any) { setErro(e.message ?? "Erro ao registrar recebimento.") }
    finally { setSalvando(false) }
  }

  const enderecoStr = formatEndereco(fiado.endereco)
...
function FiadoCard({ fiado, onReceber }: { fiado: FiadoEmAberto; onReceber: (f: FiadoEmAberto) => void }) {
  ...
  return (
    <div style={{ ...s.fiadoCard, borderColor }}>
      ...
      <div style={s.linhas}>
        <div style={s.linha}><span style={s.lLabel}>Produtos</span><span style={s.lValor}>{fiado.itens.map(i => `${i.quantidade}× ${i.produto_title}`).join(", ")}</span></div>
        {enderecoStr && <div style={s.linha}><span style={s.lLabel}>Endereço</span><span style={{ ...s.lValor, fontSize: "11px" }}>📍 {enderecoStr}</span></div>}
        <div style={s.linha}><span style={s.lLabel}>Data da venda</span><span style={s.lValor}>{formatData(fiado.data_venda)}</span></div>
        {fiado.data_pagamento_vale && (
          <div style={s.linha}>
            <span style={s.lLabel}>Vencimento</span>
            <span style={{ ...s.lValor, color: atrasado ? "#dc2626" : C.texto }}>{formatData(fiado.data_pagamento_vale)}</span>
          </div>
        )}
        <div style={{ ...s.linha, borderBottom: "none", fontWeight: 600, fontSize: "15px" }}>
          <span>Total</span><span>{formatMoney(fiado.valor_total)}</span>
        </div>
      </div>
      {jaRecebido ? (
        <div style={s.avisoAguardando}>✓ Recebimento registrado por {fiado.recebido_por_nome ?? "você"}. Aguardando confirmação do gerente.</div>
      ) : (
        <button style={s.btnReceber} onClick={() => onReceber(fiado)}>Receber {formatMoney(fiado.valor_total)}</button>
      )}
    </div>
  )
}
...
export default function RecebimentoFiadoTela({ token, usuario }: Props) {
  ...
  function selecionarCliente(c: ClienteBusca) {
    setClienteSelecionado(c); setModo("cliente_fiados"); setBusca(""); setClientes([])
  }
  ...
  const fiadosDoCliente = clienteSelecionado ? meusFiados.filter(f => f.cliente_id === clienteSelecionado.id) : []
      ...
      {!carregando && modo === "cliente_fiados" && clienteSelecionado && (
        <>
          <div style={s.clienteHeader}>
            <button style={s.btnVoltarCliente} onClick={() => { setModo("lista_geral"); setClienteSelecionado(null) }}>← Voltar</button>
...
const ss: Record<string, CSSProperties> = {
  ...
  rLabel:     { color: C.textoSecundario, flexShrink: 0 },
  rValor:     { color: C.texto, fontWeight: 500, textAlign: "right" as const, maxWidth: "65%", wordBreak: "break-word" as const },
  ...
  acoes:      { display: "flex", gap: "8px", marginBottom: "4px" },
