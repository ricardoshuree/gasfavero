...
def _get_or_create_conta_motorista(session: SessionDep, motorista_id: str) -> str:
    ...
    row = conn.execute(sa.text("SELECT id FROM conta WHERE motorista_id = :mid"), {"mid": motorista_id}).fetchone()
    if row:
        return str(row[0])
    max_num = conn.execute(sa.text(
        "SELECT MAX(CAST(numero AS INTEGER)) FROM conta "
        "WHERE pai_id = :pai AND numero ~ '^[0-9]+$' AND CAST(numero AS INTEGER) >= 1101"
    ), {"pai": CONTA_TRANSITO_ID}).scalar()
    ...
    ), {"id": nova_conta_id, "numero": proximo_numero, "nome": f"Caixa em Transito - {nome}",
        "pai_id": CONTA_TRANSITO_ID, "motorista_id": motorista_id})
...
def _saldo_conta(conn, conta_id, inicio, fim):
    row = conn.execute(sa.text(
        "SELECT COALESCE(SUM(CASE WHEN credito_id = :cid THEN valor ELSE 0 END), 0) - "
...
@router.get("/status/{data}", dependencies=[Depends(require_module_permission(MODULE, action="read"))])
def read_status_abertura(session: SessionDep, data: date) -> Any:
    ...
    for m in motoristas:
        ...
        abertura = conn.execute(sa.text(
            "SELECT id, fundo_troco, created_at FROM abertura_dia WHERE motorista_id = :mid AND data = :data"
        ...
        if abertura:
            log_rows = conn.execute(sa.text(
                ...
                'FROM abertura_dia_log adl LEFT JOIN "user" u ON u.id = adl.editado_por_id '
                'WHERE adl.abertura_id = :abertura_id ORDER BY adl.editado_em DESC'
            ), {"abertura_id": str(abertura[0])}).fetchall()
            for lr in log_rows:
                logs.append({
                    "campo": lr[0], "valor_anterior": lr[1], "valor_novo": lr[2],
                    "editado_em": lr[3].isoformat(), "editado_por": lr[4] or lr[5] or "?",
                })
        resultado.append({
            "motorista_id": motorista_id, "motorista_nome": nome,
            "aberto": abertura is not None, "fechado": fechamento is not None,
...
@router.get("/abertura/{abertura_id}/produtos",
...
def read_abertura_produtos(session: SessionDep, abertura_id: str) -> Any:
    conn = session.connection()
    todos = conn.execute(sa.text("SELECT id, title FROM item ORDER BY title")).fetchall()
    ...
    return [{"produto_id": str(p[0]), "produto_nome": p[1], "quantidade": carga_map.get(str(p[0]), 0)} for p in todos]


@router.post("/abertura", dependencies=[Depends(require_module_permission(MODULE, action="create"))])
def criar_abertura(*, session: SessionDep, current_user: CurrentUser, body: dict) -> Any:
    ...
    _criar_lancamento(session, data=data_abertura, descricao="Abertura do dia - fundo de troco",
        valor=fundo_troco, debito_id=CONTA_MESTRE_ID, credito_id=conta_motorista_id,
        abertura_id=abertura_id, criado_por_id=str(current_user.id))
    for p in body.get("produtos", []):
...
@router.patch("/abertura/{abertura_id}",
    ...
    if novo_fundo_raw is not None:
        ...
        if diferenca != 0:
            ...
            _gravar_log_edicao(conn, abertura_id=abertura_id, campo="fundo_troco",
                valor_anterior=f"R$ {fundo_atual:,.2f}", valor_novo=f"R$ {novo_fundo:,.2f}",
    ...
    if produtos_novos is not None:
        ...
        for p in produtos_novos:
            ...
            if qtd_nova == qtd_atual:
                continue
            _gravar_log_edicao(conn, abertura_id=abertura_id, campo=f"produto:{nome_produto}",
                valor_anterior=str(qtd_atual), valor_novo=str(qtd_nova), editado_por_id=editor_id)
            if qtd_nova == 0:
                conn.execute(sa.text(
                ...
                conn.execute(sa.text(
                    "INSERT INTO abertura_dia_produto (id, abertura_id, produto_id, quantidade) VALUES (:id, :aid, :pid, :qtd)"
                ), {"id": str(uuid.uuid4()), "aid": abertura_id, "pid": pid, "qtd": qtd_nova})
            else:
                conn.execute(sa.text(
                    "UPDATE abertura_dia_produto SET quantidade = :qtd WHERE abertura_id = :aid AND produto_id = :pid"
...
def verificar_senha_gerente(*, session: SessionDep, body: dict) -> Any:
    ...
    gerente = conn.execute(sa.text(
        'SELECT u.id, u.hashed_password, u.is_active FROM "user" u '
    ...
    if not gerente: raise HTTPException(status_code=403, detail="Usuario nao encontrado ou sem permissao")
    if not gerente[2]: raise HTTPException(status_code=403, detail="Usuario inativo")
    if not verify_password(senha, gerente[1]): raise HTTPException(status_code=403, detail="Senha incorreta")
    return {"autorizado": True, "gerente_id": str(gerente[0])}


@router.get("/resumo/{motorista_id}/{data}",
    dependencies=[Depends(require_module_permission(MODULE, action="read"))])
def read_resumo_fechamento(session: SessionDep, motorista_id: str, data: date) -> Any:
    ...
    t = {k: Decimal("0") for k in ["dinheiro","pix","cartao_debito","cartao_credito","vale"]}
    for forma, valor in totais:
        if forma in t: t[forma] = Decimal(str(valor))
    ...
    return {
        ...
        "total_esperado": float(fundo_troco + t["dinheiro"]),
        "total_geral": float(sum(t.values())),
...
@router.post("/fechar", dependencies=[Depends(require_module_permission(MODULE, action="create"))])
def fechar_dia(*, session: SessionDep, current_user: CurrentUser, body: dict) -> Any:
    ...
    conn = session.connection()
    if conn.execute(sa.text("SELECT id FROM fechamento_dia WHERE motorista_id = :mid AND data = :data"),
    ...
    if not abertura: raise HTTPException(status_code=404, detail="Abertura nao encontrada")
    totais = conn.execute(sa.text(
        "SELECT forma_pagamento, SUM(valor_pago) FROM venda "
        "WHERE motorista_id = :mid AND data_venda = :data GROUP BY forma_pagamento"
    ...
    conta_motorista = conn.execute(sa.text("SELECT id FROM conta WHERE motorista_id = :mid"),
                                   {"mid": motorista_id}).fetchone()
    if not conta_motorista: raise HTTPException(status_code=400, detail="Conta do motorista nao encontrada")
    cmi = str(conta_motorista[0])
    ...
    if total_contado > 0:
        _criar_lancamento(session, data=data_fechamento, descricao="Fechamento - dinheiro fisico entregue",
            valor=total_contado, debito_id=cmi, credito_id=CONTA_MESTRE_ID,
            abertura_id=abertura_id, criado_por_id=str(current_user.id))
    if t["pix"] > 0:
        _criar_lancamento(session, data=data_fechamento, descricao="Fechamento - Pix recebido",
            valor=t["pix"], debito_id=cmi, credito_id=CONTA_MESTRE_ID,
            abertura_id=abertura_id, criado_por_id=str(current_user.id))
    if t["cartao_debito"] > 0:
        _criar_lancamento(session, data=data_fechamento, descricao="Fechamento - cartao debito (maquininha)",
            valor=t["cartao_debito"], debito_id=cmi, credito_id=CONTA_MAQUININHA_ID,
            abertura_id=abertura_id, criado_por_id=str(current_user.id))
    if t["cartao_credito"] > 0:
        _criar_lancamento(session, data=data_fechamento, descricao="Fechamento - cartao credito (maquininha)",
            valor=t["cartao_credito"], debito_id=cmi, credito_id=CONTA_MAQUININHA_ID,
            abertura_id=abertura_id, criado_por_id=str(current_user.id))
    if abs(diferenca) >= Decimal("0.01"):
        if diferenca < 0:
            _criar_lancamento(session, data=data_fechamento,
                descricao=f"Quebra de caixa - {justificativa[:100]}",
                valor=abs(diferenca), debito_id=CONTA_QUEBRA_ID, credito_id=cmi,
                abertura_id=abertura_id, criado_por_id=str(current_user.id))
        else:
            _criar_lancamento(session, data=data_fechamento,
                descricao=f"Sobra de caixa - {justificativa[:100]}",
                valor=diferenca, debito_id=cmi, credito_id=CONTA_SOBRA_ID,
                abertura_id=abertura_id, criado_por_id=str(current_user.id))
    for r in body.get("retorno_produtos", []):
...
@router.get("/dashboard", dependencies=[Depends(require_module_permission(MODULE, action="read"))])
def read_dashboard(session: SessionDep, periodo: str = "hoje") -> Any:
    hoje = date.today()
    if periodo == "semana":
        data_inicio = hoje - timedelta(days=(hoje.weekday() + 1) % 7)
    elif periodo == "mes":
    ...
    for ct in contas_transito:
        ...
        motoristas_saldo.append({
            "motorista_id": mid, "motorista_nome": nome, "iniciais": iniciais, "status": status,
            "fundo_troco": float(abertura[1]) if abertura else 0, "saldo_transito": s,
        })

    for tm in conn.execute(sa.text(SQL_MOTORISTAS)).fetchall():
        mid = str(tm[0])
        if mid in ids_com_conta: continue
        nome = tm[1] or tm[2]
        iniciais = "".join(p[0].upper() for p in nome.split()[:2])
        abertura = conn.execute(sa.text(
            "SELECT id FROM abertura_dia WHERE motorista_id = :mid AND data = :data"
        ...
        motoristas_saldo.append({
            ...
            "status": "aberto" if abertura else "sem_abertura", "fundo_troco": 0, "saldo_transito": 0.0,
        })
    ...
    lancamentos = conn.execute(sa.text(
        ...
        "JOIN conta cd ON cd.id = lc.debito_id JOIN conta cc ON cc.id = lc.credito_id "
        "WHERE lc.data >= :inicio AND lc.data <= :fim "
        "ORDER BY lc.created_at DESC LIMIT 15"
    ), {"inicio": data_inicio, "fim": hoje}).fetchall()

    return {
        "periodo": periodo, "data_inicio": data_inicio.isoformat(), "data_fim": hoje.isoformat(),
        "resumo": {"saldo_mestre": saldo_mestre, "total_transito": total_transito,
